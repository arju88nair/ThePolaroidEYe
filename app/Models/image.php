...
class image extends Eloquent
{
    ...
    public static function index($input)
    {
        ...
        if (isset($_FILES['upl']) && $_FILES['upl']['error'] == 0) {
            ...
            if (move_uploaded_file($_FILES['upl']['tmp_name'], 'uploads/' . $_FILES['upl']['name'])) {
                $model            = new self();
                $url              = $_FILES['upl']['name'];
                $hue              = self::Hue($url); // Fetching the hue calculating function
                $model->hue       = round($hue);
                $model->url       = url('/') . '/uploads/' . $url;
                $model->IPaddress = $_SERVER['REMOTE_ADDR']; //Saving the ip address of the user for obvious reasons!
                $issaved          = $model->save(); //saving it to the database
                if ($issaved) {
                    echo '{"status":"success"}';
                    exit;
                }
    ...
    {
        // A simple method to get the datas for the ajax call with the specified hue and spread !
        $offset = $input['spread'];
        $hue    = $input['hue'];
        $max    = (int) $hue + (int) $offset;
        $min    = (int) $hue - (int) $offset;
         $collections = self::whereBetween('hue', [$min, $max])->get();
        // The best part of the elopquent query for mongodb.I find it much simple than normal SQL query.
        //These queries made me love MongoDb-Laravel connections
    ...
    public static function deleteCollections()
    {
